...
async function handler(m, { sock }) {
    ...
    if (!keyword) {
        let msg = `*CARI FITUR*\n\n`
        msg += `Cara Pakai:\n`
        msg += `- ${m.prefix}carifitur <keyword>\n\n`
        msg += `Contoh:\n`
        msg += `- ${m.prefix}carifitur sticker\n`
        msg += `- ${m.prefix}carifitur download\n`
        msg += `- ${m.prefix}carifitur game`
        return m.reply(msg)
    }
    ...
    try {
        ...
        for (let i = 0; i < Math.min(5, topMatches.length); i++) {
            ...
            const badges = []
            if (p.isPremium) badges.push('[P]')
            if (p.isOwner) badges.push('[O]')
            
            const badgeStr = badges.length ? ` ${badges.join('')}` : ''
            
            text += `*[ ${i + 1}. ${m.prefix}${p.name}${badgeStr} ]*\n`
            text += `- Kategori: ${p.category.toUpperCase()}\n`
            text += `- Deskripsi: ${p.description.slice(0, 50)}${p.description.length > 50 ? '...' : ''}\n`
        ...
        if (topMatches.length > 5) {
            text += `+${topMatches.length - 5} hasil lainnya tersedia.\n`
            text += `Gunakan tombol di bawah untuk melihat pilihan lain.`
        }
        
        const buttons = topMatches.slice(0, 10).map((p) => ({
            title: `${m.prefix}${p.name}`,
            description: `${p.category.toUpperCase()} • ${p.description.slice(0, 40)}`,
            id: `${m.prefix}${p.name}`
        }))
