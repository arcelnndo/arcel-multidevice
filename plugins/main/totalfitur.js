...
async function createPremiumChart(categories, totalCommands, enabledCommands) {
    ...
    ctx.fillText('DISTRIBUSI FITUR', 40, 45)
    
    ctx.fillStyle = THEME.textSecondary
    ctx.font = '12px Arial'
    ctx.fillText(`${config.bot?.name || 'ᴀʀᴄᴇʟᴢ ᴍᴜʟᴛɪ ᴅᴇᴠɪᴄᴇ'} • Command Statistics`, 40, 72)
    
    const statBoxes = [
        { label: 'TOTAL', value: totalCommands, color: THEME.primary },
        { label: 'ENABLED', value: enabledCommands, color: THEME.success },
        { label: 'KATEGORI', value: sorted.length, color: THEME.secondary }
    ]
    ...
    statBoxes.forEach((stat, i) => {
        ...
        ctx.shadowBlur = 10
        ctx.fillText(`${stat.value}`, statX + i * 125 + 57, 42)
        ctx.shadowBlur = 0
    ...
    topCategories.forEach(([cat, data], i) => {
        ...
        ctx.textAlign = 'right'
        ctx.textBaseline = 'middle'
        ctx.fillText(`${cat.toUpperCase()}`, barStartX - 12, y + barHeight / 2)
...
async function handler(m, { sock }) {
    try {
        ...
        let caption = `*DISTRIBUSI FITUR BOT*\n\n`
        caption += `*STATISTIK*\n`
        ...
        caption += `\nTotal: ${totalCommands} fitur tersedia.`
