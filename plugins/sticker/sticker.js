...
async function handler(m, { sock, config: botConfig }) {
    ...
    if (!isImage && !isVideo) {
        await m.reply(
            `*🖼️ STICKER MAKER*\n\n` +
            `Alat ini digunakan untuk membuat stiker dari gambar atau video.\n\n` +
            `*Cara pakai:*\n` +
            `Kirim atau reply gambar/video dengan perintah \`${m.prefix}s\`\n\n` +
            `*Opsi Tambahan:*\n` +
            `- \`--crop\` : Memotong media menjadi persegi\n` +
            `- \`--resize WxH\` : Mengubah ukuran (Contoh: 256x256)\n` +
            `- \`--circle\` : Memotong media menjadi bentuk lingkaran\n` +
            `- \`--rounded\` : Membuat sudut media menjadi melengkung\n\n` +
            `*Contoh:*\n` +
            `${m.prefix}s --crop\n` +
            `${m.prefix}s --circle\n` +
            `${m.prefix}s ARCELZAI NandoDev`
        )
        return
    }
    ...
    try {
        ...
        if (isVideo) {
            ...
            try {
                ...
                if (duration > 10) {
                    await m.reply(`*❌ VIDEO TERLALU PANJANG*\n\nDurasi video: ${duration.toFixed(1)} detik\nMaksimal: 10 detik.`)
                    await m.react('❌')
        ...
        const packname = options.packname || botConfig.sticker?.packname || botConfig.bot?.name || 'ARCELZAI MULTI DEVICE'
        const author = options.author || botConfig.sticker?.author || botConfig.owner?.name || 'Nando Dev'
