...
caption += `HP: ${user.rpg.health}/${user.rpg.maxHealth} • MP: ${user.rpg.mana}/${user.rpg.maxMana} • SP: ${user.rpg.stamina}/${user.rpg.maxStamina}\n\n`

caption += `*💰 ASET & KEUANGAN*\n`
caption += `Koin: Rp ${user.koin?.toLocaleString('id-ID') || 0}\n`
caption += `Bank: Rp ${user.rpg.bank?.toLocaleString('id-ID') || 0}\n`
caption += `Energi: ${isOwner || isPremium ? '∞' : user.energi}\n`

if (user.rpg.spouse) {
    caption += `Pasangan: @${user.rpg.spouse.split('@')[0]}\n`
}
