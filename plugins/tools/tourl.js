...
async function handler(m, { sock }) {
    ...
    if (m.quoted?.message) {
        ...
        if (!type || type === 'conversation' || type === 'extendedTextMessage') {
            return m.reply('*⚠️ PERINGATAN*\n\nHarap reply pesan media (gambar/video/audio).')
        }
        try {
            ...
            return m.reply(`*❌ GAGAL*\n\nGagal mengunduh media: ${e.message}`)
        }
    } else if (m.message) {
        const type = getContentType(m.message)
        if (!type || type === 'conversation' || type === 'extendedTextMessage') {
            return m.reply(`*⚠️ CARA PAKAI*\n\nKirim media dengan caption \`${m.prefix}tourl\` atau reply media yang sudah ada.`)
        }
        try {
    ...
    if (!media || media.length === 0) {
        return m.reply('*⚠️ MEDIA KOSONG*\n\nMedia tidak dapat ditemukan.')
    }
    ...
    let caption = `*📤 HASIL MULTI UPLOAD*\n\n`
    caption += `Ukuran: ${(media.length / 1024 / 1024).toFixed(2)} MB\n`
    caption += `Berhasil: ${results.length}/${UPLOADERS.length}\n\n`
    ...
    await sock.sendMessage(m.chat, {
        ...
        contextInfo: {
            externalAdReply: {
                title: '📤 Multi Upload',
                body: `Berhasil diunggah ke ${results.length} host`,
                thumbnail,
