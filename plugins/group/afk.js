...
async function handler(m, { sock }) {
    const reason = m.text || 'Tanpa alasan'
    setAfkUser(m.sender, reason)
    await m.reply(
        `*💤 AFK DIAKTIFKAN*\n\n` +
        `User: @${m.sender.split('@')[0]}\n` +
        `Alasan: ${reason}\n\n` +
        `_Bot tidak akan mengganggu Anda. Kirim pesan apa pun untuk menonaktifkan mode AFK._`,
        { mentions: [m.sender] }
    )
}
...
async function checkAfk(m, sock) {
    ...
    if (afkData) {
        ...
        await m.reply(
            `*👋 AFK BERAKHIR*\n\n` +
            `@${m.sender.split('@')[0]} telah kembali online.\n` +
            `Durasi AFK: ${duration}`, 
            { mentions: [m.sender] }
        )
    }
    ...
    if (m.isGroup && m.mentionedJid && m.mentionedJid.length > 0) {
        for (const mentioned of m.mentionedJid) {
            ...
            if (mentionedAfk) {
                ...
                await m.reply(
                    `*💤 USER SEDANG AFK*\n\n` +
                    `Jangan ganggu @${mentioned.split('@')[0]} dulu.\n` +
