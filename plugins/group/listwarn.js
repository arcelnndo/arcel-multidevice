...
async function handler(m, { sock }) {
  ...
  if (targetUser) {
    ...
    if (userWarnings.length === 0) {
      await m.reply(`*✅ INFO*\n\n@${targetName} tidak memiliki catatan pelanggaran (warning) di grup ini.`, {
        mentions: [targetUser],
      });
      return;
    }

    let txt = `*⚠️ DETAIL WARNING @${targetName}*\n\n`;
    txt += `Total Pelanggaran: ${userWarnings.length}/3\n\n`;

    userWarnings.forEach((w, i) => {
      const date = timeHelper.fromTimestamp(w.time, "DD/MM/YYYY");
      txt += `${i + 1}. Alasan: ${w.reason}\n`;
      txt += `   Waktu: ${date}\n`;
    });
    ...
    if (usersWithWarnings.length === 0) {
      await m.reply(`*✅ INFO*\n\nTidak ada anggota yang memiliki catatan pelanggaran di grup ini.`);
      return;
    }

    let txt = `*⚠️ DAFTAR PELANGGARAN GRUP*\n\n`;
    ...
    txt += `\n_Gunakan perintah ${m.prefix}listwarn @user untuk melihat detail alasan setiap pelanggaran._`;
