...
async function handler(m, { sock, jadibotId, isJadibot }) {
    ...
    if (isList) {
        if (isJadibot && jadibotId) {
            ...
            if (jbPremiums.length === 0) {
                return m.reply(`*💎 DAFTAR PREMIUM JADIBOT*\n\nBelum ada pengguna premium yang terdaftar.\nGunakan \`${m.prefix}addprem\` untuk menambahkan.`)
            }
            let txt = `*💎 DAFTAR PREMIUM JADIBOT*\n\n`
            txt += `Bot: ${jadibotId}\n`
    ...
    if (isJadibot && jadibotId) {
        if (isAdd) {
            if (addJadibotPremium(jadibotId, targetNumber)) {
                m.react('💎')
                return m.reply(
                    `*💎 PREMIUM JADIBOT DITAMBAHKAN*\n\n` +
                    `Bot: ${jadibotId}\n` +
                    `Nomor: ${targetNumber}\n` +
                    `Total: ${getJadibotPremiums(jadibotId).length} premium`
                )
            } else {
                return m.reply(`*❌ GAGAL*\n\nNomor \`${targetNumber}\` sudah terdaftar sebagai premium di Jadibot ini.`)
            }
        } else if (isDel) {
            if (removeJadibotPremium(jadibotId, targetNumber)) {
                m.react('✅')
                return m.reply(
                    `*✅ PREMIUM JADIBOT DIHAPUS*\n\n` +
                    `Bot: ${jadibotId}\n` +
                    `Nomor: ${targetNumber}\n` +
                    `Total: ${getJadibotPremiums(jadibotId).length} premium`
                )
            } else {
                return m.reply(`*❌ GAGAL*\n\nNomor \`${targetNumber}\` bukan pengguna premium di Jadibot ini.`)
            }
        }
        return
    }
    ...
    if (isAdd) {
        ...
        if (existingIndex !== -1) {
            ...
            message = `Premium berhasil diperpanjang`
        } else {
        ...
        if (index === -1) {
            return m.reply(`*❌ GAGAL*\n\nNomor \`${targetNumber}\` bukan pengguna premium.`)
        }
