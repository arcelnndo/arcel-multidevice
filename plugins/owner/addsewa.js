...
async function handler(m, { sock }) {
  ...
  if (args.length < 2) {
    return m.reply(
      `*📝 ADD SEWA*\n\n` +
        `Gunakan fitur ini untuk menyewakan bot ke sebuah grup.\n\n` +
        `*Format:*\n` +
        `${m.prefix}addsewa <link/id> <durasi>\n\n` +
        `*Format Durasi:*\n` +
        `- \`7d\` = 7 hari\n` +
        `- \`1m\` = 1 bulan\n` +
        `- \`1y\` = 1 tahun\n` +
  ...
  if (!expiredAt) {
    return m.reply(`*❌ GAGAL*\n\nFormat durasi tidak valid.\nContoh yang benar: \`7d\`, \`1m\`, \`1y\`, \`lifetime\``);
  }
  ...
  try {
    ...
    if (!result) {
      m.react("❌");
      return m.reply(`*❌ GAGAL*\n\nGrup tidak ditemukan atau link tautan tidak valid.`);
    }
    ...
    return m.reply(
      `*✅ SEWA DITAMBAHKAN*\n\n` +
        `Grup: ${groupName}\n` +
        `ID: ${groupId.split("@")[0]}\n` +
        `Durasi: ${formatDuration(durationStr)}\n` +
        `Expired: ${expiredStr}`
    );
  } catch (error) {
    m.react("❌");
